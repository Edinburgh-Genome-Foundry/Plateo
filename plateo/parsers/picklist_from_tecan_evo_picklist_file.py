    ...
    for i, row in df.iterrows():
        if row.Action == "A":
            ...
            source_well_name = index_to_wellname(
                int(row.Position), num_wells=source_plate.num_wells, direction="column"
            )
            source_well = source_plate[source_well_name]
        if row.Action == "D":
            dest_plate = plates_dict[row.RackLabel]
            dest_well_name = index_to_wellname(
                int(row.Position), num_wells=dest_plate.num_wells, direction="column"
            )
            dest_well = dest_plate[dest_well_name]
            picklist.add_transfer(source_well, dest_well, volume=float(row.Volume))
    return picklist
